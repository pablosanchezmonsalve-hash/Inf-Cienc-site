...
export function analisisResultados(pubs, sel, meta, pais) {
  ...
  // recorte apuntaba a `produccion.htmlanio=2025`, que no existe.
  const q = X.consulta(sel || {}) ? `?${X.consulta(sel || {})}` : '';
  ...
  if (I.percentil.base && top10) {
    /* Sin redondear, con un decimal, como el `esperado` de I-05 en el build
       (134,2). Redondeado, un recorte de 3 decía «sería 0» y comparaba contra
       ese cero. */
    const esperado = I.percentil.base * 0.10;
  ...
  if (C.internacional.base) {
    ...
    col.push(hallazgo('Colaboración internacional',
      (inter
        ? `${npub(inter)} (${c.num(C.internacional.valor, 1)} %) ${inter === 1 ? 'tiene' : 'tienen'} autores de más de un país.`
          + (C.socios.length ? ` Aparte de ${e(pais || 'la institución')}, los países más frecuentes en las afiliaciones son `
            + `${C.socios.map(s => `${e(s.valor)} (${nf(s.n)})`).join(', ')}.` : '')
        : 'Ninguna publicación del recorte tiene autores de más de un país.'),
  ...
  if (C.internacional.base) {
    partes.push(C.internacional.valor
      ? `el ${c.num(C.internacional.valor, 1)} % de lo publicado es en colaboración internacional`
